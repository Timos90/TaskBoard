""" 
This file is going to handle all the authentication logic.
It is going to receive a request from the frontend that includes some details(jwt token) about the signed in user,
our backend is going to authenticate if these details are valid, we are going to get the user details from clerk and check the user's permissions.
 """
import httpx
...
async def get_current_user(request:Request) -> AuthUser:
    ...
    request_state = clerk.authenticate_request(
        ...
        options=AuthenticateRequestOptions(authorized_parties=[settings.FRONTEND_URL])
    )
...
""" 
The functions below are going to be used as dependencies in the API routes to check if the user has the required permissions
 """
